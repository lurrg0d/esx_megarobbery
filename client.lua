...
local keyToInteractWithRobbery = Keys["F"]
--[[--------]]
			...
			if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceForMarkerToShow and isRobbing==false and robbableSpot.beingRobbed==false)then
				...
				if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceToInteractWithMarker)then
					...
					if(robbableSpot.isSafe)then
						DisplayHelpText("Tryck ~INPUT_CONTEXT~ för att börja bryta upp skåpet.")
					else
						DisplayHelpText("Tryck ~INPUT_CONTEXT~ för att börja bryta upp kassan.")
					end
			...
			Citizen.CreateThread(function()
				while isRobbing do
					if(spotBeingRobbed.isSafe)then
						DisplayHelpText("Du bryter nu upp kassaskåpet ("..spotBeingRobbed.timeToRob-currentSecondCount.." återstår)")
					else
						DisplayHelpText("Du håller nu på att bryta upp ("..spotBeingRobbed.timeToRob-currentSecondCount.." återstår)")
					end
					Citizen.Wait(0)
				end
			end)
...
function RobberyOver()			
	DisplayHelpText("Du har framgångsrikt rånat stället!")
...
AddEventHandler("robberies:robberyOverNotification", function(name)
		TriggerEvent("pNotify:SendNotification", {
			text = "10-90 : Någon flydde platsen vid ( "..name.." )",
			type = "warning",
			timeout = 10000,
			layout = "centerRight",
			queue = "right"
...
AddEventHandler("robberies:robberyStartedNotification", function(name)
		TriggerEvent("pNotify:SendNotification", {
			text = "10-90 : Ett tyst alarm har gått igång ( "..name.." )",
			type = "warning",
			timeout = 10000,
			layout = "centerRight",
			queue = "right"
